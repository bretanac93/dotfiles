...
_ssh_interactive() {
  ...
  # Upstream names this `argv`, which in zsh is an alias for the positional
  # parameters — assigning to it would clobber "$@" before the loop reads it.
  local -a all_args=("$@")
  ...
  while (($#)); do
    ...
    if [[ -z $opts_done && $arg == "--" ]]; then
      ...
      for ((i = 0; i < ${#letters}; i++)); do
        # zsh needs the offset expanded: ${letters:i:1} is a modifier error.
